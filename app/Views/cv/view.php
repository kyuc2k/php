...
$name = trim($cv['name'] ?? '') ?: $ownerName;
$ownerPictureFinal = $cvPhotoUrl ?: $ownerPicture;
$email = trim($cv['email'] ?? '');
$phone = trim($cv['phone'] ?? '');
$address = trim($cv['address'] ?? '');
$linkedin = trim($cv['linkedin'] ?? '');
$github = trim($cv['github'] ?? '');
$website = trim($cv['website'] ?? '');
...
$activeSections = array_filter($sections, fn($_, $k) => trim($cv[$k] ?? '') !== '', ARRAY_FILTER_USE_BOTH);
...
<div class="cv-wrapper">
    ...
    <aside class="panel-left">
        <?php if ($ownerPictureFinal): ?>
        <img src="<?= htmlspecialchars($ownerPictureFinal) ?>" alt="<?= htmlspecialchars($name) ?>" class="avatar avatar-img" referrerpolicy="no-referrer">
        <?php else: ?>
    ...
    <main class="panel-right">
        ...
        <div class="failed-box">
            ...
            <?php if ($parseError): ?>
            <p>
                <strong>Lỗi Gemini API:</strong> <?= htmlspecialchars($parseError) ?>
            </p>
            <?php else: ?>
            <p>
